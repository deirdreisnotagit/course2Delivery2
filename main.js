...
const createViolin = (data) => {
  ...
  // Append the area (density plot) for men
    ...
    svg
      .append('path')
        ...
        .attr('stroke', 'none')
        .style('filter', 'url(#glow)'); // Added during milestone 4;
  ...
  const offsetTooltip = 20;
  const formatMoney = d3.format('.4s');
  // Show the tooltip when hovering a circle
  const handleMouseOver = (event, d) => {
    ...
    tooltip.select('.total-earnings-section .total-earnings').text(formatMoney(d.earnings_USD_2019));
  
   // Position and reveal tooltip
   tooltip
   .style('top', `${event.pageY + offsetTooltip}px`) 
   .style('left', `${ event.pageX  + offsetTooltip}px`) 
